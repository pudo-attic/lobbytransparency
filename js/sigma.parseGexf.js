...
sigma.publicPrototype.parseGexf = function(gexf) {
  ...
  for(i=0; i<nodesNodes.length; i++){
    ...
    for(j=0; j<nodeNodes.length; j++){
      ...
      for(k=0; k<attvalueNodes.length; k++){
        ...
        var attr = nodesAttributes[attvalueNode.getAttribute('for')];
  ...
  sigmaInstance.numNodes = nodes.length;
  ...
  for(i=0; i<edgesNodes.length; i++){
    ...
    for(j=0; j<edgeNodes.length; j++){
      ...
      for(k=0; k<attvalueNodes.length; k++){
        var attvalueNode = attvalueNodes[k];
        var attr = edgesAttributes[attvalueNode.getAttribute('for')];
        var val = attvalueNode.getAttribute('value');
  ...
  sigmaInstance.numEdges = edges.length;
};
